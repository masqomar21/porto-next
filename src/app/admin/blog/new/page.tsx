...
export default function NewBlogPostPage() {
  ...
  const handleSave = async () => {
    ...
    if (!form.slug) { setToast({ type: 'error', msg: 'Slug is required.' }); return; }
    setSaving(true);
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/admin/blog/${data._id}/edit`);
      } else {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground">New Blog Post</h1>
        <p className="text-muted-foreground text-sm mt-1">Create a new article or tutorial</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div className="space-y-6">
          ...
          />
          <RichTextEditor content={form.content} onChange={(html) => set('content', html)} placeholder="Write your post here…" />
        </div>
