...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[1000] h-20 flex items-center px-6 md:px-12 transition-all duration-300 ${
        scrolled ? 'bg-background/90 backdrop-blur-md border-b border-foreground/10' : 'bg-transparent border-b border-transparent'
      }`}>
        <Link href="/" className="font-serif italic font-bold text-2xl tracking-tight text-foreground hover:opacity-80 transition-opacity">
          Portfolio.
        </Link>
        ...
        <div className="flex items-center gap-4 md:gap-8 ml-auto">
          ...
          <Link href="/admin" className="hidden md:inline-block px-5 py-2.5 bg-foreground text-background hover:opacity-90 rounded-full text-xs font-mono uppercase tracking-widest transition-all">
            Admin
          </Link>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[998] bg-background/60 backdrop-blur-sm md:hidden" onClick={() => setMenuOpen(false)} />
      )}

      {/* Mobile Menu Drawer */}
      <div className={`fixed inset-y-0 right-0 z-[999] w-full sm:w-80 bg-background border-l border-foreground/10 p-6 pt-24 flex flex-col gap-6 shadow-2xl md:hidden transition-transform duration-300 ${
